...
def associate_product_ce(product_loc, targets):
    ...
    for id, target in targets.items():
        ...
        if target.left_hand is not None and target.right_hand is not None:
            ...
            ce_distance =   calculate_distance3D(head, product_loc)*hscore + \
                            calculate_distance3D(left_hand, product_loc)*lscore + \
                            calculate_distance3D(right_hand, product_loc)*rscore
        else:
            ce_distance = calculate_distance3D(target.head['position'], product_loc)
        # print("Distance for target: ", id, "is: ", str(distance))
